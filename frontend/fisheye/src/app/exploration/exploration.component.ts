...
export class ExplorationComponent implements OnInit {
  constructor(private ds: APIService, public es: ExplorationService) {
    this.sliderDisplay = this.sliderDisplay.bind(this);
  }
  private map!: L.Map;
  private canvas: any;
  private faoSVG: any;
  ...
  private faoURL =
    'https://www.fao.org/fishery/geoserver/fifao/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=fifao%3AFAO_AREAS_ERASE_LOWRES&maxFeatures=50&outputFormat=application%2Fjson';
  ...
  sliderMax: number =
    ...
    1; // the total number of dats in the dataset
  sliderVal: number = 1;
  mapScale: string = 'log';
  ...
  ngOnInit(): void {
    ...
    this.legend = d3.select('#legend').attr('height', 360).attr('width', 90);
    ...
    this.es.setInterval(
      this.dateRangeToInterval(new Date(start), new Date(end))
    );
  ...
  sliderDisplay(value: number): string {
    // const that = this;
    const date = this.valToDate(value);
    return this.dateToStr(date);
  } // Displays the date as a string on the slider thumb
  ...
  faoChange(event: any) {
    ...
    if (this.faoChecked) {
      ...
      features.on('pointermove', mousemove).on('pointerout', mouseleave);
      ...
      function mouseleave(d: any) {
        that.faoTooltip.style('visibility', 'hidden');
      }
    } else if (!this.faoChecked) {
      if (!this.faoSVG.selectAll('g').empty())
        this.faoSVG.selectAll('g').remove();
    }
  }
}
